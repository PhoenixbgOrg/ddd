      ...
      <div className="container mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div className="max-w-6xl mx-auto">
          <div className="border-b border-gray-300 flex justify-center space-x-2">
            <button onClick={() => setActiveView('mix')} className={navButtonClasses('mix')}>
              Калкулатор за смеси
            </button>
            <button onClick={() => setActiveView('history')} className={navButtonClasses('history')}>
              История на партидите
            </button>
            <button onClick={() => setActiveView('label')} className={navButtonClasses('label')}>
              Генератор на етикети
            </button>
             {currentUser.role === 'Администратор' && (
               <button onClick={() => setActiveView('admin')} className={navButtonClasses('admin')}>
                  Админ Панел
                </button>
             )}
          </div>
