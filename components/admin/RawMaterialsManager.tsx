...
const RawMaterialsManager: React.FC = () => {
    ...
    const loadData = useCallback(() => {
        try {
            const { definitions: loadedDefinitions, lots: loadedLots } = ensureDefaultRawMaterialsAndLots();
    ...
    const saveDefinitions = (newDefs: RawMaterialDefinition[]) => {
        ...
        localStorage.setItem('ddd_raw_material_definitions', JSON.stringify(newDefs));
    }
    const saveLots = (newLots: RawMaterialLot[]) => {
        setLots(newLots);
        localStorage.setItem('ddd_raw_material_lots', JSON.stringify(newLots));
    }
...
const DefinitionManager: React.FC<DefinitionManagerProps> = ({ definitions, saveDefinitions, selectedDefinitionId, setSelectedDefinitionId, allLots, saveLots }) => {
    ...
    const handleDelete = (id: string) => {
        if(window.confirm('Сигурни ли сте? Това ще изтрие дефиницията и всички свързани с нея партиди!')) {
            ...
            // Also delete associated lots - This should be handled in the parent or via a callback
        }
    }
...
const LotManager: React.FC<LotManagerProps> = ({ lots, allLots, saveLots, selectedDefinitionId, selectedDefinitionName }) => {
    const defaultFormState = { lotNumber: '', supplier: '', receivedDate: new Date().toISOString().split('T')[0], expiryDate: '', initialGrams: 0, pricePerKg: 0 };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if(isEditing) {
            saveLots(allLots.map(l => l.id === isEditing ? {...l, ...form, initialGrams: Number(form.initialGrams), pricePerKg: Number(form.pricePerKg)} : l));
        } else {
            saveLots([...allLots, { 
                ...form, 
                id: `lot-${Date.now()}`,
    ...
    const handleEdit = (lot: RawMaterialLot) => {
        setIsEditing(lot.id);
        setForm({lotNumber: lot.lotNumber, supplier: lot.supplier, receivedDate: lot.receivedDate, expiryDate: lot.expiryDate, initialGrams: lot.initialGrams, pricePerKg: lot.pricePerKg});
    }
            ...
            <div style={{borderColor: 'red'}} className="border-2 p-4 rounded-lg">
                ...
                <form onSubmit={handleSubmit} className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        ...
                        <input type="number" step="0.01" placeholder="Цена (лв/кг)" value={form.pricePerKg} onChange={e => setForm({...form, pricePerKg: Number(e.target.value)})} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500" />
